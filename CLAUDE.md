...
services/           # Docker Compose stacks (13 categories: 12 DocoCD-managed + gitops)
molecule/           # Ansible role testing (one scenario per role)
.github/            # CI workflows and shared composite actions
```

Service categories: ai, home_assistant, media, monitoring, storage, tools, dashboards, security, photos, backups, networking, games. Plus `gitops/` — the DocoCD container itself, which stays Ansible-managed (it can't redeploy itself without killing the in-flight deploy).
...
- The `gitops/` stack is the exception — it hosts DocoCD itself and is updated via Ansible (see `roles/services/tasks/gitops.yml`).
...
- The `services` role is now bootstrap-only: install Docker, create `home_server_network`, plant the SOPS age key + cloudflared tunnel token + `services_data/` dir, render the `gitops/` stack's `.env` from vault. Nothing under it handles runtime deploys.
...
- `gitops/` stack's own `.env` (contains `WEBHOOK_SECRET` + `SOPS_AGE_KEY` for the DocoCD container) is templated by Ansible from `vault_services_env.gitops` — it can't be SOPS-encrypted because DocoCD itself needs it at startup.
